...
def search_duplicate():
    ...
    df['Intermediate Description']=df['ID1'].astype(str)+": "+df['Idea Title']
    df['Intermediate Description'] = df['Intermediate Description'].apply(lambda x: (x[:100] + '...') if len(x) > 100 else x)
